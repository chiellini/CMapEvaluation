...
class HeatMap:
    def __init__(self, image, heat_map, backgroud,gaussian_std=5):
        ...
        # masked_heatmap_image_resized=np.ma.masked_where(heatmap_image_resized==0,heatmap_image_resized)
    ...
    def save(self, filename, format='png', save_path=os.getcwd(),
        ...
        plt.imshow(self.image)
        plt.imshow(self.heat_map / 255, alpha=transparency, cmap=color_map)
        masked_background=np.ma.masked_where(self.backgroud!=1,self.backgroud)
        # print(masked_background)
