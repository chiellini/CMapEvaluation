...
from utils.image_io import nib_load, check_folder
...
def four_metrics_evaluate(args=None):
    ...
    for gdth_fpath, pred_fpath in tqdm(zip(gt_files, pred_files), desc=f"Calculating Dice and Hausdorff Distance",
        ...
        sw_slice = int(gdth_np.shape[-1] / 2)
        gdth_np=gdth_np[:,:,sw_slice]
        # tmp=np.unique(gdth_np)
    ...
    for embryo_tp in embryo_wise_effective_mean_surface_dis_list.keys():
        len_mean_surface=len(embryo_wise_effective_mean_surface_dis_list[embryo_tp])
        len_hausdorff=len(embryo_wise_effective_hausdorff_distance95_list[embryo_tp])
        if len_hausdorff<1 or len_mean_surface<1:
            pd_fourmetrics_embryo_wise.loc[len(pd_fourmetrics_embryo_wise)] = [embryo_tp,None,None,None,None,1]
        else:
            pd_fourmetrics_embryo_wise.loc[len(pd_fourmetrics_embryo_wise)] = [embryo_tp,
                                                                               sum(
                                                                                   embryo_wise_effective_mean_surface_dis_list[
                                                                                       embryo_tp]) / len_mean_surface,
                                                                               sum(
                                                                                   embryo_wise_effective_hausdorff_distance95_list[
                                                                                       embryo_tp]) / len_hausdorff,
                                                                               sum(
                                                                                   embryo_wise_effective_dice_score_list[
                                                                                       embryo_tp]) / len(
                                                                                   embryo_wise_effective_dice_score_list[
    ...
    pd_fourmetrics_embryo_wise.loc[len(pd_fourmetrics_embryo_wise)] = ['AllCellAverage',
                                                                       ...
                                                                       sum(
                                                                           ...
                                                                           all_effective_jaccard_score_list),
                                                                       sum(all_embryo_difference_list) / sum(
                                                                           all_embryo_gt_cell_num_dict.values())
                                                                       ]
...
if __name__ == "__main__":
    args = dict(
        gt_folder=r"C:\Users\zelinli6\OneDrive - City University of Hong Kong - Student\Documents\06paper TUNETr TMI LSA NC\TUNETr dataset\Timelapse2DsliceEvaluationGT\200113plc1p2",
        pred_folder=r"C:\Users\zelinli6\OneDrive - City University of Hong Kong - Student\Documents\06paper TUNETr TMI LSA NC\TUNETr dataset\ForTimelapseAnd2DEvaluation\2DTimelapse\StarDist3D",
        method_name='StarDist3D',
