...
# source_folder = r"C:\Users\zelinli6\OneDrive - City University of Hong Kong - Student\Documents\06paper TUNETr TMI LSA NC\TUNETr dataset\3DEvaluation"
# # change this when you are working on validation
unmatched_cell_volume_files_path = glob(os.path.join(
    r'C:\Users\zelinli6\OneDrive - City University of Hong Kong - Student\Documents\04paper CMap coroperation\A first revision\gt_cell_volume_verification\origianl_gt_volumes',
    "*.nii.gz"))  # change this when you are working on validation

# change this when you are working on validation
target_files_path = glob(os.path.join(
    r'C:\Users\zelinli6\OneDrive - City University of Hong Kong - Student\Documents\04paper CMap coroperation\A first revision\gt_cell_volume_verification\seg_volumes',
    "*.nii.gz"))
unmatched_cell_volume_files_path.sort()
target_files_path.sort()

saving_path_root=r'C:\Users\zelinli6\OneDrive - City University of Hong Kong - Student\Documents\04paper CMap coroperation\A first revision\gt_cell_volume_verification\match_gt_cell_volumes'
assert len(unmatched_cell_volume_files_path) == len(target_files_path), "#files not equal"

bar = tqdm(range(len(unmatched_cell_volume_files_path)))
bar.set_description("Uniform labels")
for pred_file_path, target_file_path in zip(unmatched_cell_volume_files_path, target_files_path):
    this_mapping_dict = {}
    ...
    nib_save(
        os.path.join(saving_path_root,os.path.basename(save_file)),
