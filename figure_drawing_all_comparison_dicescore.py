...
evaluation_dice_score_table=pd.DataFrame(columns=['EmbryoName','MethodName','AvgDiceScore'])
for embryo_name_tmp in embryo_names:
    for method_name_tmp in method_name_list:
        ...
        dice_score_list_tmp=list(dice_score_pd['DiceScore'])
        avg_dice_score_this=sum(dice_score_list_tmp)/len(dice_score_list_tmp)
        evaluation_dice_score_table.loc[len(evaluation_dice_score_table.index)]=[embryo_name_tmp,method_name_tmp,avg_dice_score_this]

evaluation_dice_score_table.to_csv('Table S - DiceScoreEvaluation.csv',index=False)
